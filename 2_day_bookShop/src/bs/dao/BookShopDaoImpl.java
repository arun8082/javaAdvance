...
public class BookShopDaoImpl implements IBookShopDao {
	...
	public BookShopDaoImpl() throws Exception {
		...
		pst5 = con.prepareStatement("SELECT id,title,author,price FROM dmc_books WHERE id=?");
	}
	...
	@Override
	public Books getBooksDetailsById(String bid) throws Exception {
		pst5.setInt(1, Integer.parseInt(bid));
		ResultSet rst = pst5.executeQuery();
		Books book = null;
		if (rst.next()) {
			book = new Books(rst.getInt("id"), rst.getString("title"), rst.getString("author"), rst.getDouble("price"));
		}
		return book;
	}

	@Override
	public List<Books> addBooksInList(List<Books> oldList, Books book) throws Exception {
		oldList.add(book);
		return oldList;
	}
	
	@Override
	public List<Books> getBooksListsByBookIds(String[] ids) throws Exception {
		ArrayList<Books> cartBooksList = new ArrayList<>();
		String idList = "";
		if (ids.length != 0) {
			for (String id : ids) {
				idList += id + ",";
		...
		pst4 = con.prepareStatement("SELECT id,title,author,price FROM dmc_books WHERE id IN(" + idList + ")");
		// System.out.println(pst4);
		ResultSet rst = pst4.executeQuery();
		while (rst.next()) {
			cartBooksList.add(new Books(rst.getInt("id"), rst.getString("title"), rst.getString("author"),
					rst.getDouble("price")));
		}
